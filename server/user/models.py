...
class UserManager(BaseUserManager):
    def create_user(self, email, password=None, **kwargs):
        ...
        # Handle eth_wallet field
        eth_wallet = kwargs.get('eth_wallet', '')
    ...
    def create_superuser(self, email, password, **kwargs):
        ...
        user.first_name = kwargs.get('first_name', 'Default first name')
        user.last_name = kwargs.get('last_name', 'Default last name')
        user.is_admin = True
        user.is_staff = True
...
class User(AbstractBaseUser):
    ...
    eth_wallet = models.CharField(max_length=42, blank=True, null=True)  # Ethereum wallet address
...
@receiver(post_save, sender=get_user_model())
def save_avatar(sender, instance=None, **kwargs):
    # Ensure that this function does not run on every save
    if os.environ.get('RUN_MAIN') == 'true':
        return
    
    # Handle the avatar fetching logic
